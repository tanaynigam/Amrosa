...
const IMPORT_SYSTEM_INSTRUCTION =
  "You are a recipe parser for the ChefsJournal recipe app. " +
...
const FREEFORM_SYSTEM_INSTRUCTION =
  "You are a recipe formatter for the ChefsJournal recipe app. " +
...
/**
 * Format a freeform recipe typed by the user into structured ChefsJournal schema.
 *
...
async function fetchGoogleSheet(sheetId) {
  ...
  try {
    const response = await axios.get(exportUrl, {
      timeout: 15000,
      headers: { "User-Agent": "Mozilla/5.0 (compatible; ChefsJournal-RecipeParser/1.0)" },
      maxRedirects: 5,
    });
...
async function callGemini(content, sourceHint, apiKey, systemInstruction) {
  ...
  const prompt =
    `Parse the following recipe content into ChefsJournal's recipe JSON format.\n\n` +
    `Source: ${sourceHint}\n\n` +
...
const CONVERT_SYSTEM_INSTRUCTION =
  "You are a unit-conversion assistant for the ChefsJournal recipe app. " +
