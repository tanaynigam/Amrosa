...
| Gap | Detail |
|---|---|
| **🔁 Rebrand → Chef's List (`com.aerion.chefslist` / `chef-s-list`)** | **Android + shared infra are fully rebranded; iOS is still on the old identity.** The iOS session must: (1) change the **bundle id** `com.aerion.amrosa` → `com.aerion.chefslist`; (2) add an iOS app to the **`chef-s-list`** Firebase project and swap in the new **`GoogleService-Info.plist`**; (3) change the custom URL **scheme** `amrosa://` → `chefslist://` and the **Associated Domains** host to `applinks:chef-s-list.web.app` (the hosted AASA already lists `com.aerion.chefslist`); (4) rename the Xcode **project/target/folder** `Amrosa` → `ChefsList` and set the display name to "Chef's List"; (5) update any hardcoded web-client-id / project refs. Data is already migrated to `chef-s-list` with UIDs preserved (`auth:import`), so signing in with the same Google account restores everything. |
