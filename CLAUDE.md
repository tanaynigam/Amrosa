...
| **F15 — Ingredient quantity ranges** | Android-only. iOS needs: 3 fields on the SwiftData ingredient model — `quantityValueMax`, `quantityValueMaxMetric`, `quantityValueMaxImperial` (all `Double?`, default nil; SwiftData adds columns automatically — no manual migration); the scaler renders "min–max unit" scaling **both** ends when `quantityValueMax > quantityValue` (mirror `QuantityScaler.scale`); thread the 3 fields through every Firestore push/pull map and the parsed-import mapping. **No client conversion math** — the backend already returns the metric/imperial maxes (`computeMaxConversions`). The editor should let the user set a range max. |
...
| **Collective step-ingredient linking (cooking-mode fallback)** | Android-only on-device half. iOS needs the same fallback when showing a step's ingredients in cooking mode: any ingredient referenced by **no** step in its section should attach to that section's first step (mirror `augmentedStepRefs()`), so collectively-referenced ingredients ("add all the paste ingredients") still appear. The import-side prompt + `linkOrphanIngredients` server net are shared backend (already covers fresh imports). |
...
| **F13 — Discover tab (full)** | Whole feature: `cooked_log` + ranker + popularity/search + `DiscoverView`; **Discover is the default + left-most tab** (tab order `Discover · My Recipes · Shared · Account`). Shared backend deployed. See above. |
