...
| **Discover tab (F13 feed)** | Default + left-most tab. `MealClassifier` + `DiscoverRanker` (meal match · implicit cuisine affinity · source boost · recency penalty; popularity term reads 0 until Phase 5). Shelves: "{Meal} ideas", "From your kitchen", "From your co-chefs" (`getAuthorRecipes` per co-chef), "Fresh from the community" (`getPublicRecipeSummaries`), "Recently cooked" (`cooked_log`). Surprise-me + pull-to-refresh. `CookedLogModel` + `markCooked()` on Cooking Mode "Done!". Local card → editable detail; remote → `ReceivedRecipeView(.direct)`. *(Popularity/likes shelf, cross-scope search, explicit cuisine prefs = Phases 5–7, still pending.)* |
...
| Gap | Detail |
|---|---|
| **F13 — Discover popularity/search/prefs (Phases 5–7)** | Feed foundation + tab is ported (see implemented section). Remaining: **Phase 5** popularity — `getPopularPublicRecipes`, `setLiked`/`likeStateFlow` + ❤ on the review screen, a "Popular" shelf; **Phase 6** cross-scope search (`searchPublicRecipes` via `searchTokens`); **Phase 7** explicit cuisine prefs (`UserPreferences`) overriding implicit affinity. The Cloud Functions, rules, indexes, and `searchTokens` in `buildDocument` are already deployed (shared infra). |
