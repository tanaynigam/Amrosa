...
| **F18 — Notes = community thread (Comments merged in)** | The detail screen shows a single **"Notes"** section on **shared recipes** (any tier — `notesVisible = visibility != private`), backed by the existing `shared_recipes/{id}/comments` subcollection (the `Comment` model/flow/post/delete reused, UI relabeled). **Anyone who can view may add**; **delete by note author or recipe owner**. Owner has a **Lock** toggle (`toggleNotesLock` → `notesLocked` on the mirror doc, merge-written so republish never clears it; `SharedRecipeService.setNotesLocked`/`getNotesLocked`); when locked the input is hidden and the rule blocks creates (`get(...).data.get('notesLocked', false) == false`). The old **private on-device notes** (`RecipeNoteEntity`) section is **removed from the detail screen** (entity/`addNote`/`deleteNote` linger unused — clean up later). **Deploy `firestore:rules`.** **Android only** (iOS pending). |
...
| **F18 — Notes = community thread** | Android-only. iOS should merge its **Comments** + private **Notes** into one **"Notes"** section shown on shared recipes (any tier), backed by the existing comments subcollection (anyone who can view may add; note-author or recipe-owner deletes). Add the owner **Lock** toggle → `notesLocked` on the mirror doc (merge-written so republish keeps it) + hide the input when locked. Rules (notes-create gated by `notesLocked`) are shared infra. |
