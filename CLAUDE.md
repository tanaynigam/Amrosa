...
| **F19 — Edit QOL: drag-reorder + substitute adjacency** | **Drag-to-reorder** ingredients & steps in edit mode via the `sh.calvin.reorderable` lib (`rememberReorderableLazyListState(listState)` → `RecipeDetailViewModel.onDragMove(fromKey, toKey)` which dispatches to `reorderIngredient`/`reorderStep` by detecting the dragged id's type; per-section, cross-type/unknown targets are no-ops). Each editable ingredient/step row gains a leading `DragHandle` (`Modifier.draggableHandle()`); the jiggle affordance is suppressed while dragging; a dragged **step renders compacted** (`CompactStepRow`, one line) so long steps are easy to move. **Substitute adjacency:** `linkSubstitute` now moves the substitute to sit immediately after its target within the section. **Both platforms** (iOS: substitute-adjacency move + Up/Down reorder in the edit sheets + cross-section moves via the F21 Section selector; true drag-to-reorder stays an Android nicety). |
