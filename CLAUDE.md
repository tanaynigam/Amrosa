# Chef's List — Project Specification
### by Aerion

> **Chef's List** is the app name — a play on *chef's kiss*. The launch screen shows the **Chef's List** wordmark. The guiding identity of the app: recipes that are exquisite, elaborate, and deeply personal. (Formerly "Amrosa"; fully rebranded to the `com.aerion.chefslist` package + the `chef-s-list` Firebase project.)
...
| **Database (cloud)** | Firebase Firestore (`chef-s-list`) |
...
│   └── ChefsList/ChefsList/
│       ├── ChefsListApp.swift         # Entry point; Firebase init; AppContainer setup
...
│   │   ├── recipeSchema.js    # ChefsList JSON schema for Gemini prompt
...
├── hosting/          # Firebase Hosting (deployed to chef-s-list.web.app)
│   └── public/
...
**Design decisions:**
- **Auth is mandatory.** `ChefsListNavGraph` observes `authStateFlow()`. When `currentUser == null` or anonymous, the full-screen `AuthScreen` is shown (no back button, no bottom bar). The main `Scaffold` + `NavHost` don't render until sign-in.
...
- **Sync on sign-in**: `ChefsListApplication.authStateFlow` observer calls `syncPersonalRecipes()` + `syncReceivedRecipes()` whenever a real user is detected.
...
The app **requires** a signed-in account. `ChefsListNavGraph` observes `authStateFlow()` at the outermost level:
...
1. Calls `app.container.clearAllLocalData(context)` — runs Room `database.clearAllTables()` + clears `chefslist_sync` SharedPreferences
2. Calls `authRepository.signOut()`
3. `authStateFlow` emits null → `ChefsListNavGraph` recomposes to auth gate automatically
...
#### Launch sequence (`ChefsListApplication.onCreate()`)
```kotlin
container = AppContainer(this)
createNotificationChannel()   // creates "chefslist_social" NotificationChannel (Android 8+)
appScope.launch {
...
#### Firebase config
Project: `chef-s-list`. Web client ID in `res/values/strings.xml` as `google_web_client_id`. SHA-1 debug fingerprint registered in Firebase Console for Google Sign-In.
...
**Option B — "Share link"**:
- **Recipe is already public** → Android system share sheet opens with `https://chef-s-list.web.app/shared/{recipeId}`
...
Share URL format: **`https://chef-s-list.web.app/shared/{recipeId}`**

**Android App Links** — `AndroidManifest.xml` has two intent filters on `MainActivity`:
1. `android:autoVerify="true"` HTTPS filter for `chef-s-list.web.app/shared/**` — verified via `assetlinks.json` in Firebase Hosting. When verified, tapping the link opens the app directly with no chooser dialog.
2. `chefslist://shared` custom scheme — legacy fallback; works immediately without domain verification.

NavGraph composable for `"shared/{recipeId}"` handles both:
```kotlin
deepLinks = listOf(
    navDeepLink { uriPattern = "https://chef-s-list.web.app/shared/{recipeId}" },
    navDeepLink { uriPattern = "chefslist://shared/{recipeId}" }
)
```

**Firebase Hosting** (`chef-s-list.web.app`) — deployed:
- `/.well-known/assetlinks.json` — Android App Links domain verification (SHA-256 fingerprint of debug keystore)
- `/shared/{recipeId}` → `shared.html` — browser fallback page; fetches recipe from Firestore REST API, renders full recipe. Has "Open in ChefsList" button (tries `chefslist://` scheme). Works for anyone — app not required.
...
1. `apple-app-site-association` at `hosting/public/.well-known/apple-app-site-association` (`appID 7S2FY6WF5V.com.aerion.chefslist`, paths `/shared/*`); served as `application/json` via a `firebase.json` header. **Requires `firebase deploy --only hosting` to go live.**
2. `Associated Domains` entitlement `applinks:chef-s-list.web.app` in `ChefsList.entitlements`.
3. `ContentView.onOpenURL` already routes `https://chef-s-list.web.app/shared/{id}` → Shared tab (so no app code change was needed beyond the entitlement).

Test Android App Links via ADB:
```
adb shell am start -W -a android.intent.action.VIEW -d "https://chef-s-list.web.app/shared/RECIPE_ID" com.aerion.chefslist
```
...
`chefslist://shared/{recipeId}` or `https://chef-s-list.web.app/shared/{recipeId}` → opens this screen. Reads from Firestore `shared_recipes`, not Room. Features:
...
**`ChefsListMessagingService`** (`service/ChefsListMessagingService.kt`) — extends `FirebaseMessagingService`:
...
- Notification channel ID: `"chefslist_social"`, name: `"ChefsList"`, importance: DEFAULT
...
2. Sends FCM via `admin.messaging().send()` with `android.notification.channelId = "chefslist_social"`
...
#### Phase 3b — Polish ✅
- **Explicit cuisine preferences**: `data/UserPreferences.kt` (SharedPreferences `chefslist_prefs`,
...
- **Discover is now the default tab**: `ChefsListNavGraph` `startDestination = BottomTab.Discover.route`.
...
| Background Sync | On-launch coroutine (`ChefsListApplication`) |
| Recipe AI | Gemini 2.5 Flash via Firebase Cloud Functions v2 (Node.js 24) |
| Image Loading | Coil |
| DI | Manual (`AppContainer` in `ChefsListApplication`) |
...
| Hosting | Firebase Hosting (`chef-s-list.web.app`) |
...
| **Firebase Firestore** | Connected (`chef-s-list`), security rules deployed |
...
| **F8 — Share button** | Top bar icon (owners only); if public → Android share sheet with `chefslist://shared/{id}`; if private → dialog → publish → share sheet |
| **F8 — Deep links + App Links** | HTTPS App Links (`https://chef-s-list.web.app/shared/{id}`) + `chefslist://` fallback; `assetlinks.json` in Firebase Hosting; `navDeepLink` for both patterns in NavGraph |
| **F8 — Firebase Hosting** | `shared.html` recipe viewer (browser fallback); `index.html` landing page; deployed at `chef-s-list.web.app` |
...
| **F9 — FCM push notifications** | `ChefsListMessagingService` (`onNewToken` stores token, `onMessageReceived` shows foreground notification); `sendPushNotification` Cloud Function triggered on `notifications/{uid}/items` creation; channel `"chefslist_social"` created on app start; Co-Chef / recipe share push text |
...
| **Notification deep links** | Tapping a push routes to the target screen: `recipe_shared` → `received/{shareId}`; follow notifications → Account tab. `MainActivity` reads FCM extras → `ChefsListApplication.pendingDeepLink` → nav graph navigates. |
...
| **F8 — Share link** | iOS share sheet with `https://chef-s-list.web.app/shared/{id}`; publish dialog for private recipes |
...
| **Deep links** | `chefslist://shared/{id}` custom scheme + `https://chef-s-list.web.app/shared/{id}` via `onOpenURL`; routes to `SharedRecipeDetailView` |
...
| Gap | Detail |
|---|---|
| **🔁 Rebrand → Chef's List (`com.aerion.chefslist` / `chef-s-list`)** | ✅ **Code/config done in the repo** (compiles with code-signing disabled): bundle id → `com.aerion.chefslist`; display name → "Chef's List"; custom scheme `amrosa://` → `chefslist://`; deep-link host + share URLs → `chef-s-list.web.app`; `Associated Domains` → `applinks:chef-s-list.web.app`; Xcode **project/target/folders renamed** `Amrosa` → `ChefsList` (`ios/ChefsList/ChefsList/…`, `ChefsListApp`, `ChefsList.entitlements`, `ChefsListTests`, `project.yml`); dead Amrosa color palette removed; `amrosaCard()`→`cardStyle()`; UserDefaults keys → `chefslist_*`; notification names → `ChefsList*`. **⚠️ Two steps require the user's accounts (can't be done in-repo):** (a) **Firebase** — add an iOS app (`com.aerion.chefslist`) to the `chef-s-list` project, download the new **`GoogleService-Info.plist`** (replace `ios/ChefsList/ChefsList/GoogleService-Info.plist`, still the old amrosa-2ec82 one), and update the **REVERSED_CLIENT_ID** URL scheme in `project.yml` (currently the old one, annotated); (b) **Signing** — the `com.aerion.chefslist` App ID needs the **Associated Domains + Push Notifications** capabilities enabled (Xcode automatic signing prompts for this on first real build; the CI build only failed on provisioning, not compilation). Data is already migrated to `chef-s-list` with UIDs preserved, so signing in with the same account restores everything. |
...
| **Universal Links** | ✅ Wired — `Associated Domains` entitlement (`applinks:chef-s-list.web.app`) + AASA at `hosting/public/.well-known/apple-app-site-association` (served as application/json). Pending `firebase deploy --only hosting`. |
...
### Navigation
- **Auth gate**: `ChefsListNavGraph` collects `authStateFlow()`. `isSignedIn = currentUser?.isAnonymous == false`. When false → renders `AuthScreen()` (root, no back). When true → renders `MainAppScaffold()`.
...
- The `"auth"` route is **removed** from the nav graph — auth is handled at the outer `ChefsListNavGraph` level.
...
- Deep link `chefslist://shared/{recipeId}` is registered on the `"shared/{recipeId}"` composable via `navDeepLink`.
...
- On sign-in: `ChefsListApplication.authStateFlow` observer automatically triggers seed + sync.
...
- **FCM token** — `ChefsListMessagingService.onNewToken()` stores token in `users/{uid}.fcmToken`. `refreshFcmToken()` in `ChefsListApplication` fetches current token on each sign-in. `updateFcmToken(token)` in `SocialRepository` writes the field.
...
### AppContainer
- `clearAllLocalData(context: Context)` — `database.clearAllTables()` + clears `chefslist_sync` prefs. Called on sign-out.
...
| Project generation | xcodegen 2.45.4 (`project.yml` in `ios/ChefsList/`) |
| Bundle ID | `com.aerion.chefslist` |
| Firebase project | chef-s-list |

### iOS File Structure

```
ios/ChefsList/
├── project.yml                         # xcodegen spec (run: xcodegen generate from ios/ChefsList/)
├── ChefsList.xcodeproj/                   # generated — do not edit manually
└── ChefsList/
    ├── ChefsListApp.swift                 # @main, Firebase.configure(), AppContainer init
...
| **F8 — Visibility chip** | Owners see Private/Public toggle chip in detail body; confirms before toggling |
| **F8 — Share button** | Top bar icon (owners only); if public → iOS share sheet with `https://chef-s-list.web.app/shared/{id}`; if private → dialog → publish → share sheet |
...
| **Deep links** | `chefslist://` custom URL scheme registered in project.yml; `https://chef-s-list.web.app/shared/{id}` handled via `onOpenURL`; routes to `SharedRecipeDetailView` |
...
- **xcodegen**: After adding/removing Swift files, run `xcodegen generate` from `ios/ChefsList/` to regenerate the Xcode project. Never edit `.xcodeproj` manually.
...
- **Deep links**: `onOpenURL` in ContentView handles both `chefslist://shared/{id}` and `https://chef-s-list.web.app/shared/{id}`. Sets `deepLinkRecipeId` binding which `SharedRecipesView` uses to navigate to `SharedRecipeDetailView`.
