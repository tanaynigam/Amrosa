...
| `shared_recipes/{recipeId}` | Mirror for shared recipes. `visibility` field = `"friends"` (Co-Chefs only) or `"public"` — **never `"private"`** (rules reject create/update unless visibility ∈ {friends, public}). Read gated by Firestore rules: public → anyone; friends → author + accepted co-chefs (F12). The mirror is reconciled with the canonical recipe: making a recipe private, or deleting it (any shared tier), removes the mirror; `saveEdit` re-publishes or unpublishes to match. F13: `saveCount`/`likeCount` counters (Cloud-Function-maintained) + `likes/{uid}` subcollection. |
...
| **Privacy: no private recipes in Discovery/profiles** | Fixed stale `shared_recipes` mirrors leaking into Discovery/profile (private must be owner-only, friends → co-chefs, public → all). Rules now reject `shared_recipes` create/update unless `visibility ∈ {friends, public}`. Client reconciles the mirror: delete now unpublishes any shared tier (was public-only), `saveEdit` unpublishes when private. One-off `backend/firestore/cleanup-private-mirrors.js` purges existing orphans (deletes mirror docs whose canonical recipe is missing or private). **Deploy `firestore:rules` + run cleanup script.** Read rules already blocked private docs; this stops orphans being created/lingering. |
