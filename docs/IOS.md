...
> (*rebrand(ios): Amrosa → Chef's List*). Its `GoogleService-Info.plist` is still the **old
> `amrosa-2ec82`** file. See the **🔁 Rebrand** row in the gaps table below for exactly what is required.
...
| Gap | Detail |
|---|---|
| **🔁 Rebrand → Chef's Journal (`com.aerion.chefsjournal` / `chef-s-journal-6a0fd`)** | ⛔ **NOT done on iOS.** iOS currently sits on the **Chef's List** identity (`ios/ChefsList/`, bundle id `com.aerion.chefslist`, commit `14d2c52`). Android + all shared infra (Firestore rules, hosting, assetlinks, Cloud Functions) are already on **`chef-s-journal-6a0fd`**, and the **data + auth users are fully migrated there with UIDs preserved** — so iOS is pointing at a project that is no longer the source of truth. iOS must: (1) bundle id → **`com.aerion.chefsjournal`**; (2) add an iOS app to **`chef-s-journal-6a0fd`** and replace `ios/ChefsList/ChefsList/GoogleService-Info.plist` (still the original **amrosa-2ec82** file) + update **REVERSED_CLIENT_ID** in `project.yml`; (3) scheme `chefslist://` → **`chefsjournal://`**; (4) Associated Domains + deep-link/share host → **`chef-s-journal-6a0fd.web.app`** (hosted assetlinks/AASA already list the chefsjournal ids); (5) rename Xcode project/target/folders `ChefsList` → `ChefsJournal`, display name **"Chef's Journal"** (identifiers `ChefsJournal`/`chefsjournal`, **no apostrophe**); (6) enable **Associated Domains + Push Notifications** on the new App ID. Then sign in with the same Google account — UIDs are preserved, so all data returns. |
