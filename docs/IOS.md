...
> ✅ **Rebrand complete (verified on disk).** iOS is fully on the **Chef's Journal** identity:
> `ios/ChefsJournal/`, bundle id `com.aerion.chefsjournal`, and a `GoogleService-Info.plist`
> pointing at **`chef-s-journal-6a0fd`** — matching Android and the shared backend.
> Commits: `67bbfa4` (rename + deep links), `b32314c` (Google Sign-In reversed client id).
>
> 🧹 **Leftover to delete:** a stray `ios/ChefsList/` directory survived the rename (1 tracked
> file, ~5 KB). It is dead weight — remove it so there is only one iOS tree.
>
> ⚠️ **Identifier names elsewhere in this file may still be unreliable.** Earlier bulk
> find/replace passes during the Android rename also rewrote iOS names in this document.
> Trust the repo over the prose here.
...
| Gap | Detail |
|---|---|
| **🔁 Rebrand → Chef's Journal** | ✅ **Done in the repo — verified on disk.** Bundle id `com.aerion.chefsjournal`; display name "Chef's Journal"; scheme `chefslist://` → `chefsjournal://`; deep-link/share host + Associated Domains → `chef-s-journal-6a0fd.web.app`; project/target/folders renamed `ChefsList` → `ChefsJournal` (`ios/ChefsJournal/ChefsJournal/…`, `ChefsJournalApp`, `ChefsJournal.entitlements`, `ChefsJournalTests`, `project.yml` regenerated); UserDefaults keys → `chefsjournal_*`; notification names → `ChefsJournal*`. **`GoogleService-Info.plist` is the new `chef-s-journal-6a0fd` file** and `REVERSED_CLIENT_ID` is wired from it (commits `67bbfa4`, `b32314c`). **⚠️ Still needs the user's Apple account:** register the new App ID with **Associated Domains + Push Notifications**, and upload the **APNs auth key** to `chef-s-journal-6a0fd` or push won't deliver on iOS. A new bundle id = a new App Store Connect app, as planned. UIDs were preserved in the migration, so signing in restores all data. |
